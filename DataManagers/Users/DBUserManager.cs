    ...
    public class DBUserManager : IUserManager
    {
        ...
        public override User AddUser()
        {
            ...
            try
            {
                ...
                using (var db = new MovieContext())
                {
                    ...
                    var occupationForUser =
                        db.Occupations.FirstOrDefault(c => c.Name.ToLower() == occupation.ToLower());
            ...
            try
            {
                using (var db = new MovieContext())
                {
                    ...
                    var user = db.Users.FirstOrDefault(c =>
                        c.Age == age && c.Gender == gender && c.Occupation == chosenOc && c.ZipCode == zipcode);
                    if (user == null)
                    {
                        logger.Debug(
                            $"User not found for user Parameters UserAge:{age} UserGender:{gender} UserOccupation:{chosenOc.Name}");
        ...
        public override void ShowUserRatings()
        {
            ...
            try
            {
                ...
                using (var db = new MovieContext())
                {
                    var userRatings = db.UserMovies.Include(c => c.Movie).Include(c => c.User)
                    ...
                    foreach (var x in occupations)
                    {
                        ...
                        userRatingsTable.AddRow(movie?.Movie.Title == null ? "No Movie Info" : movie.Movie.Title,
                            movie?.Rating == null ? "N/A" : movie.Rating, x.Name);
                    }
